...
func CommandSchema() map[string]any {
	...
	data := map[string]any{}
	for _, k := range strings.Fields("name description title problem goal context owner objective rationale status body reason current commit deployment session environment_id mechanism instructions result observations logs severity url provider notes") {
		data[k] = str()
	}
