...
assert.ok(evaluate('overview()').includes('feature-list'));
assert.ok(!evaluate('overview()').includes('Environments & repositories'));
const configurationHTML=evaluate('productConfiguration()');
assert.ok(configurationHTML.includes('Environments and repositories'));
for(const tab of ['topology','components','compositions','providers','releases','workspace'])assert.ok(configurationHTML.includes(`data-config-tab=\"${tab}\"`),tab);
evaluate(`configurationTab='components'`);assert.ok(evaluate('productConfiguration()').includes('<h2>Components</h2>'));
evaluate(`configurationTab='topology'`);
evaluate(`savedFeatures=state.features;state.features=[{id:'old',product_id:'p',updated_at:'2026-09-01T00:00:00Z'},{id:'new',product_id:'p',updated_at:'2026-09-16T00:00:00Z'},{id:'fallback',product_id:'p',created_at:'2026-09-10T00:00:00Z'}]`);
assert.equal(evaluate("sortedFeatures().map(f=>f.id).join(',')"),'new,fallback,old');
evaluate('state.features=savedFeatures');
