...
Ключевые знания продукта хранятся отдельно от истории Feature/Integration.
Для поиска используй `search_product_knowledge {"product_id":"…","query":"слова"}`.
Результат содержит активные узлы и связанные рёбра. Если обнаружил новую
устойчивую договорённость, ограничение, решение или discovery, запиши её через
...
```json
{
  ...
  "actor": "agent/backend",
  "node": {
    "id": "payments-api-v1-contract",
    "kind": "contract",
    "title": "Payments API v1 remains operational",
    "content": "Existing clients cannot migrate atomically; preserve v1 callbacks.",
    "keywords": ["payments", "api", "v1", "callback"],
    "status": "active"
  }
}
```

Используй тот же `id`, чтобы исправить знание; старый снимок и автор сохраняются
в истории. Для устаревшего факта укажи `status: "archived"`. Узлы не являются
задачами, разрешениями или командами на выполнение и не должны содержать
секреты.
...
Control Plane также сам сканирует подключённые к Product кодовые репозитории (SOURCE/APPLICATION/LIBRARY/MIXED). GitOps-репозитории в эту очередь не попадают: они остаются историей желаемого состояния доставки. По умолчанию worker запускает такой refresh примерно раз в 2 минуты и читает все ветки, кроме настроенной base. Интервал меняется через `RCP_GIT_SYNC_INTERVAL` (минимум 30 секунд, `0` отключает scheduler). Наблюдения сохраняются даже без Feature/Integration: это позволяет пережить работу агента, который не успел сообщить о коммитах. Получить очередь таких кандидатов можно через `get_unlinked_git_commits {"product_id":"…"}` или `get_attention_required`; она также включена в `resume` как `unlinked_git_commits`, чтобы агент увидел её при обычном восстановлении контекста. Каждый элемент содержит репозиторий, ветки, SHA, сообщение, автора и ссылку. После проверки агент прикрепляет коммит к подходящей Integration обычным `update_integration`/записью commit provenance; наблюдение и история refresh при этом не удаляются. Отставшая или разошедшаяся ветка не попадает в Operations & attention: это состояние Git-контекста, доступное через `get_git_state` и граф фичи. Конкретная операция доставки всё равно перепроверяет свежесть и блокирует именно этот deploy, если выбранная ветка устарела или разошлась. Старые ветки можно удалять отдельной guarded Git-операцией после проверки PR/владельца.
...
Верхняя кнопка **Refresh all** запускает тот же read-only цикл вручную для
текущего Product: ставит в очередь Git всех подключённых code-repository,
наблюдение связанных активных Integration и runtime-синхронизацию всех его
Environment. Запросы выполняются параллельно и возвращают управление сразу;
кнопка не ждёт GitHub/Kubernetes, не меняет код и не выполняет deploy. Результат
появляется в общем состоянии и в истории операций после работы worker.
...
1. Для общей DEV-сборки вызови `assemble_environment` с `product_id` и `environment_id` (при необходимости передай `integration_ids`; пустой список означает все активные захваченные интеграции). Сервис сам создаст immutable composition из последних Git revisions и сразу вернёт Operation ID. Для заранее зафиксированного состава создай composition вручную и вызови `reconcile_composition` с actor и composition_id. Цель должна иметь enabled DEV/TEST mapping. Полли parent через `get_operation`, проверяй child_ids и evidence. Конфликт останавливает операцию и требует semantic resolution; после новых push запусти assemble снова.
