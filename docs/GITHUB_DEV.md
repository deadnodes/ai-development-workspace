...
Create a dedicated GitHub App (webhooks can be disabled for this polling slice). Grant repository Metadata read, Contents read/write and Actions read/write. Install it on the selected SOURCE and GITOPS repositories. Repository discovery uses the installation and stores GitHub numeric repository IDs. Discovered repositories live in the instance Repository Registry. Multiple Products can attach the same registered repository. Product attachment is explicit; an installation's visibility is not an automatic product attachment. Feature and Integration repository lists narrow that selection.
